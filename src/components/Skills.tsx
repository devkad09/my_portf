...
const Skills = () => {
  ...
  return (
    <section id="skills" className="py-20 md:py-32 px-4 sm:px-6 md:px-12 overflow-hidden relative">
      ...
      <div className="container mx-auto max-w-7xl px-4 sm:px-6 relative z-10">
        <div className="text-center mb-16 reveal">
          <p className="section-title">Core competencies</p>
          <h2 className="section-heading">The toolkit I use to ship polished web experiences.</h2>
          <p className="section-copy mx-auto mt-6 max-w-2xl">
            From frontend architecture to API design, these are the frameworks, services, and workflows I rely on every day.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 reveal">
          {categories.map((category, catIdx) => (
            <div
              key={category.name}
              className="glass-panel rounded-[3rem] border border-white/10 p-8 shadow-[0_30px_90px_-50px_rgba(0,0,0,0.55)] transition duration-500 hover:border-white/20"
            ...
            >
              <div className="flex items-center justify-between gap-4 border-b border-white/10 pb-4 mb-6">
                <h3 className="text-sm uppercase tracking-[0.35em] text-white/50 font-black">{category.name}</h3>
                <span className="rounded-full bg-accent/10 px-3 py-1 text-[10px] uppercase tracking-[0.2em] text-accent font-black">
                ...
                {category.skills.map((skill, skillIdx) => {
                  ...
                  return (
                    <button
                      ...
                      className="group flex flex-col items-center justify-center gap-3 rounded-[1.8rem] border border-white/10 bg-white/5 p-4 text-center transition duration-300 focus:outline-none"
                      style={
                        isHovered
                          ? {
                              borderColor: skill.color,
                              backgroundColor: skill.glowColor,
                              boxShadow: `0 14px 30px -14px ${skill.glowColor}`,
                              transform: "translateY(-2px)"
                            }
                          : {}
                      }
                    >
                      <div className="flex h-10 w-10 items-center justify-center text-white/80 transition group-hover:text-white">
                        {skill.icon}
                      </div>
                      <span className="text-[10px] uppercase tracking-[0.15em] text-white/70 group-hover:text-white">
                        {skill.name}
                      </span>
                    </button>
                  );
